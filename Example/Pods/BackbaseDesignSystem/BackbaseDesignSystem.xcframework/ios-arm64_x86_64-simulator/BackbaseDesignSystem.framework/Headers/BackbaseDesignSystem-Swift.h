...
/// This form is available as a view controller and is intended to be used as a child view controller within
/// a journey’s interface. The component optionally provides the capability to be configured with an
/// address-autocomplete provider, which allows suggested addresses to be injected and selected
/// by the user in order the populate the form with validated, accurate data.
/// The form consists only of the autocomplete component and the address input fields. It does not
/// include other elements that you may associate with the form, such as a submit button, which
/// you would include alongside the address form within your journey.
/// This class should <em>not</em> be instantiated directly, but with the <code>AddressFormFactory</code>.
SWIFT_CLASS("_TtC20BackbaseDesignSystem11AddressForm")
@interface AddressForm : UIViewController
